...
git clone https://github.com/miladnazarzadeh/cprd-extractor.git
cd cprd-extractor
...
| Live — Any Server | Specify a custom CPRD data root directory |
...
Streamlit Web Interface
  ...
  Dev          Lookup    Extraction  Extraction  Builder
        CPRDEngine Core (DuckDB SQL)
  Mock Data            Slurm CLI Mode
  Generator            (parallel task arrays)
Data Layer (Read-Only)
  CPRD Aurum   HES APC/OP/AE   ONS Mortality   IMD   EMIS
```
...
| Configuration | Configuration | Path management, output settings, and SSH connection panel |
...
python app.py --generate-slurm --extract-type snomed \
  ...
  --total-tasks 50 --output-dir path/to/output
...
sbatch --dependency=afterok:$JOBID cprd_snomed_merge.sh    # Merge shards
```
...
@software{nazarzadeh2026cprdextractor,
  ...
  title     = {CPRD Extractor: An Interactive Platform for Clinical Practice Research Datalink Data Extraction and Cohort Assembly},
  year      = {2026},
